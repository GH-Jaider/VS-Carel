...
demo-program.kli: finished after 12 steps
$ echo $?
0
```
...
The CLI is not on npm yet. It builds to **one file, `karel.mjs`, ~51 KB, with no
dependencies at all** — it imports `node:fs`, `node:path` and `node:util` and nothing else —
so installing it means putting that file somewhere on your `PATH`.
...
submission.kli: finished after 2 steps — expected Karel at (2, 1), found (1, 3)
```

`--assert-world` compares Karel's position, heading and bag, and every pile of beepers. It
deliberately ignores dimensions and walls: no instruction can change those, so a difference
there means the two files describe different exercises rather than a wrong answer. The
comparison only runs after a clean finish — if the program walked into a wall, that is the
finding worth reporting, not the world state it left behind.
...
| `--assert-world <file.klm>` | `-a`        | Require this final world; a mismatch exits `1`.              |
...
| `64` | `USAGE`       | Unknown flag, missing file, malformed world.                                          | Your grading script is wrong, not the student's program. Investigate before you record a zero. |

`64` is worth a special note: it is the BSD `EX_USAGE` convention, and it is the code you get
when a submission file is missing or a world is unreadable. Treating it as a failure will
silently score students for your own broken paths, so give it its own branch.
...
for f in submissions/*.kli; do
  if karel run "$f" --world start.klm --assert-world solved.klm --max-steps 10000 \
       >/dev/null 2>&1; then
...
luis.kli       FAIL  wrong result
                 luis.kli: finished after 2 steps — expected Karel at (2, 1), found (1, 3)
...
# → error	assert-world	2	expected Karel at (2, 1), found (1, 3)
...
reference.kli: finished after 12 steps
$ echo $?
0
```

Two cautions. First, only take `.world` from a run that exited `0`: a failed run still reports
the world, and freezing a half-finished one as the answer key would fail every correct
submission. Second, `solved.klm` records dimensions and walls that the comparison ignores, so
keep `start.klm` as the authority on the shape of the exercise — the answer key describes where
things end up, not what the maze looks like.
...
`src/commands.ts` holds the two commands as pure functions over file contents — nothing there
touches the filesystem or the process, so what counts as a pass is testable without spawning
anything. `src/exit.ts` is the one place the exit codes are defined, and `src/output.ts` the
one place results are rendered. `src/main.ts` is the only file that does I/O.
